...
def hello():
    ...
    data = ("bobby", "1.2.3.4")
    send_query_to_db_no_response(query, data)
    return "Hello World"
...
def return_one_devices():
    ...
    try:
        return send_query_to_db(query_specific_device,sql_data)
    except IOError:
...
def return_lamps():
    ...
    if request.method == 'GET':
        ...
        sql_data = ""
        try:
            return send_query_to_db(query_get_lamp, sql_data)
        except IOError:
            return json.dumps({'success': False}), 400, {'ContentType': 'application/json'}
    ...
    if request.method == 'POST':
        ...
        try:
            send_query_to_db_no_response(query_lamp, sql_data)
            #
...
def set_name():
    ...
    try:
        send_query_to_db_no_response(query_update_name, sql_data)
        return json.dumps({'success': True}), 200, {'ContentType': 'application/json'}
    except IOError:
        return json.dumps({'success': False}), 400, {'ContentType': 'application/json'}
...
def return_sensors():
    ...
    sql_data = ""
    try:
        return send_query_to_db(query_get_sensor,sql_data)
    except IOError:
        return json.dumps({'success': False}), 400, {'ContentType': 'application/json'}
